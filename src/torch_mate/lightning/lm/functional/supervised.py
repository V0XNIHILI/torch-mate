...
def shared_step(module: ConfigurableLightningModule, batch, batch_idx, phase: str):
    ...
    module.log(module.log_key(phase, 'loss'), loss, prog_bar=prog_bar)
    ...
    if module.hparams.learner.get("cfg", {}).get("classification", False) == True:
        if "topk" in module.hparams.learner["cfg"]:
            for i, k in enumerate(module.hparams.learner["cfg"]["topk"]):
                module.log(module.log_key(phase, 'accuracy', f"@{k}"), calc_accuracy(output, batch[1], k), prog_bar=(i == 0 and prog_bar))
        else:
            module.log(module.log_key(phase, 'accuracy'), calc_accuracy(output, batch[1]), prog_bar=prog_bar)
